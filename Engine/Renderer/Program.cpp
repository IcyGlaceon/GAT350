...
namespace cool
{
	...
	bool Program::Create(std::string filename, ...)
	{
		...
		bool success = cool::json::Load(filename, document);
		if (!success)
		{
		...
		{
			auto vshader = cool::g_resources.Get<cool::Shader>(vertex_shader, GL_VERTEX_SHADER);
			AddShader(vshader);
		}
		...
		{
			auto fshader = cool::g_resources.Get<cool::Shader>(fragment_shader, (void*)GL_FRAGMENT_SHADER);
			AddShader(fshader);
		}
